...
public partial class Sender : SingletonMono<Sender>
{
    ...
    public async FTask GameNetStart(){
        RealmScene = await Fantasy.Platform.Unity.Entry.Initialize(GetType().Assembly);
        GateScene = await Scene.Create();
    ...
    public Session Gate{
        get{
            ...
            if(session==null)
            {
                GateScene.Connect(gateAdress, NetworkProtocolType.KCP, 
                    OnGateConnected, OnConnectFail, null,false);
    ...
    public Session Realm{
        get{
            ...
            if(session==null)
            {
                RealmScene.Connect(selectZone.RealmAdress, NetworkProtocolType.KCP,
                    OnRealmConnected, OnConnectFail, null,false);
    ...
    private void OnGateConnected()
    {
        ...
        // 只需要给客户端保持连接的服务器挂心跳
        GateScene.Session.AddComponent<SessionHeartbeatComponent>().Start(2000);
        Log.Debug("已连接到网关服务器");
    }
