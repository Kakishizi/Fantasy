...
{
    protected override async FTask Run(Session session, C2G_LoginGateRequest request, G2C_LoginGateResponse response,
        Action reply)
    {
    ...
    private async FTask<uint> Check(Session session, C2G_LoginGateRequest request, G2C_LoginGateResponse response)
    {
        ...
        using (await session.Scene.CoroutineLockComponent.Wait(LockType.LoginGate, accountId,
                   "LoginGate"))
        {
            // 缓存有就获取网关账号，无则创建网关账号
