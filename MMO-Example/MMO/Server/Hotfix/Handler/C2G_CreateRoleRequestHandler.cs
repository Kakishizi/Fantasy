...
namespace BestGame
{
    ...
    public class H_G2C_CreateRoleHandler : MessageRPC<C2G_CreateRoleRequest, G2C_CreateRoleResponse>
    {
        protected override async FTask Run(Session session, C2G_CreateRoleRequest request,
            G2C_CreateRoleResponse response,
            Action reply)
        {
        ...
        private async FTask<uint> Check(Session session, C2G_CreateRoleRequest request, G2C_CreateRoleResponse response)
        {
            ...
            {
                // 锁住账号和名字
                using (await session.Scene.CoroutineLockComponent.Wait(LockType.LockGateAccount,
                           accountId, nameof(LockType.LockGateAccount)))
                {
                    // 没有账号信息
                    ...
                    Role role = Entity.Create<Role>(session.Scene,false,true);
