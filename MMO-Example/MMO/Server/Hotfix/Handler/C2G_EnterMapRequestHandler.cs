...
public class C2G_EnterMapRequestHandler : MessageRPC<C2G_EnterMapRequest,G2C_EnterMapResponse>
{
    ...
    private async FTask<uint> Check(Session session, C2G_EnterMapRequest request, G2C_EnterMapResponse response)
    {
        ...
        using (await session.Scene.CoroutineLockComponent.Wait(LockType.LockGateAccountLock,accountId,nameof(LockType.LockGateAccountLock)))
        {
            try
            {
                // 找不到账号信息
                ...
                if (sessionPlayer.EnterState == SessionState.Enter)
                {
                    ...
                    await session.Scene.NetworkMessagingComponent.SendAddressable(gateAccount.SelectRoleId,
                ...
                else
                {
                    ...
                    var result = (M2G_CreateUnitResponse)await session.Scene.NetworkMessagingComponent.CallInnerRoute(mapScene.RouteId,
