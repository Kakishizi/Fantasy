...
{
    protected override async FTask Run(Session session, C2R_LoginRequest request, R2C_LoginResponse response,
        Action reply)
    {
    ...
    private async FTask<uint> Check(Session session, C2R_LoginRequest request, R2C_LoginResponse response)
    {
        ...
        using (await session.Scene.CoroutineLockComponent.Wait(LockType.AuthAccountLock, authName.GetHashCode(),
                   nameof(LockType.AuthAccountLock)))
        {
            ...
            // 请求网关key
            var keyRsp = (G2R_GetLoginKeyResponse)await session.Scene.NetworkMessagingComponent.CallInnerRoute(
                SceneConfigData.Instance.Get(account.GateSceneId).RouteId,
            ...
            Log.Info("gateAddress: " + response.GateAddress);
